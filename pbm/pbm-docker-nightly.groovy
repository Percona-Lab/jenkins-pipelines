...
pipeline {
    ...
    parameters {
        ...
        choice(name: 'TARGET_REPO', choices: ['PerconaLab','AWS_ECR'                                        ], description: 'Target repo for docker image, use DockerHub for release only')
    }
    options {
            ...
            steps {
                withCredentials([aws(accessKeyVariable: 'AWS_ACCESS_KEY_ID', credentialsId: '8468e4e0-5371-4741-a9bb-7c143140acea', secretKeyVariable: 'AWS_SECRET_ACCESS_KEY')]) {
                     sh """
                         ...
                         docker tag percona-backup-mongodb public.ecr.aws/e7j3v3n0/psmdb-build:pbm-nightly
                         docker push public.ecr.aws/e7j3v3n0/psmdb-build:pbm-nightly
                     """
                }
            }
        }
