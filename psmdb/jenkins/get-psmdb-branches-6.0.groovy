    ...
    stages {
        stage('Get release branches') {
            steps {
                ...
                withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', accessKeyVariable: 'AWS_ACCESS_KEY_ID', credentialsId: S3_STASH, secretKeyVariable: 'AWS_SECRET_ACCESS_KEY']]) {
                    sh """
                        EC=0
                        aws s3 ls s3://percona-jenkins-artifactory/percona-server-mongodb/branch_commit_id_60.properties ${S3_ENDPOINT} || EC=\$?
			...
			if [ \${EC} = 1 ]; then
			  ...
			  aws s3 cp branch_commit_id_60.properties s3://percona-jenkins-artifactory/percona-server-mongodb/ ${S3_ENDPOINT}
                          echo "START_NEW_BUILD=NO" > startBuild
			else
                          aws s3 cp s3://percona-jenkins-artifactory/percona-server-mongodb/branch_commit_id_60.properties . ${S3_ENDPOINT}
        ...
        stage('Build needed') {
            ...
            steps {
                ...
                build job: 'hetzner-psmdb60-autobuild-RELEASE', parameters: [string(name: 'CLOUD', value: CLOUD), string(name: 'GIT_BRANCH', value: BRANCH_NAME), string(name: 'PSMDB_VERSION', value: VERSION), string(name: 'PSMDB_RELEASE', value: RELEASE), string(name: 'MONGO_TOOLS_TAG', value: MONGO_TOOLS_TAG), string(name: 'COMPONENT', value: 'testing')]
                build job: 'hetzner-psmdb-multijob-testing', propagate: false, wait: false, parameters: [string(name: 'PSMDB_VERSION', value: VERSION), string(name: 'PSMDB_RELEASE', value: RELEASE)]
            }
        }
        stage('Build skipped') {
            when {
