                ...
                withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', accessKeyVariable: 'AWS_ACCESS_KEY_ID', credentialsId: S3_STASH, secretKeyVariable: 'AWS_SECRET_ACCESS_KEY']]) {
                    sh """
                        ...
                        if [ -z "\${LATEST_RELEASE_BRANCH}" ]; then
                            ...
                            : > branch_commit_id_80.properties
                            exit 0
                        fi
                        ...
                        # PSMDB-2057: defense-in-depth via branch_commit_id_80.last_successful.
                        # Written by hetzner-psmdb80-autobuild-RELEASE's post.success block on
                        # every green run. If detected == last_successful, we KNOW we already
                        # built that branch/commit and SKIP regardless of branch_commit_id_80.properties
                        ...
                        LS_EC=0
                        AWS_RETRY_MODE=standard AWS_MAX_ATTEMPTS=10 aws s3 cp s3://percona-jenkins-artifactory/percona-server-mongodb/branch_commit_id_80.last_successful . ${S3_ENDPOINT} --cli-connect-timeout 60 --cli-read-timeout 120 || LS_EC=\$?
                        if [ \${LS_EC} = 0 ] && [ -s branch_commit_id_80.last_successful ]; then
                            LAST_SUCCESSFUL_BRANCH=\$(awk -F= '\$1=="BRANCH_NAME"{print \$2}' branch_commit_id_80.last_successful)
                            LAST_SUCCESSFUL_COMMIT_ID=\$(awk -F= '\$1=="COMMIT_ID"{print \$2}' branch_commit_id_80.last_successful)
                        fi
                        if [ -n "\${LAST_SUCCESSFUL_BRANCH}" ] \\
                           ...
                           && [ "\${LAST_SUCCESSFUL_COMMIT_ID}" = "\${LATEST_COMMIT_ID}" ]; then
                            ...
                            if [ \${EC} = 1 ]; then
                                ...
                                CACHED_BRANCH=\$(awk -F= '\$1=="BRANCH_NAME"{print \$2}' branch_commit_id_80.properties)
                                CACHED_COMMIT=\$(awk -F= '\$1=="COMMIT_ID"{print \$2}' branch_commit_id_80.properties)
