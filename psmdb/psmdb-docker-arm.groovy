            ...
            steps {
                script {
                    ...
                    def pbm_branch = sh(returnStdout: true, script: """
                        ...
                        echo \$PBM_RELEASE
                        """).trim()
                    build job: 'hetzner-pbm-functional-tests', propagate: false, wait: false, parameters: [string(name: 'PBM_BRANCH', value: pbm_branch ), string(name: 'PSMDB', value: psmdb_image ), string(name: 'instance', value: 'docker-aarch64'), string(name: 'TESTING_BRANCH', value: "pbm-${pbm_branch}")]
                }
            }
        }
    }
