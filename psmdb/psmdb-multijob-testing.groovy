        ...
        stage ('Run functional tests on tarballs') {
            steps {
                script {
                    ...
                    build job: 'psmdb-tarball-functional', propagate: false, wait: true, parameters: [ string(name: 'TARBALL', value: tarball), string(name: 'TESTING_BRANCH', value: "PSMDB_disable_debug") ]
                }
            }
        }
        stage ('Run functional tests on packages') {
            steps {
                build job: 'psmdb-parallel', parameters: [ string(name: 'REPO', value: "testing"), string(name: 'PSMDB_VERSION', value: params.PSMDB_VERSION ), string(name: 'ENABLE_TOOLKIT', value: "false"), string(name: 'TESTING_BRANCH', value: "PSMDB_disable_debug") ]
            }
        }
