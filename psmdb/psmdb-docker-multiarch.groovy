...
//   details.
//
// Please follow the described approach and **DO NOT MIX TWO COMMENT STYLES**.
...
def installSyft() {
    sh '''
        ...
        install_syft() {
            ...
            for i in 1 2 3; do
                # Notes:
                # - the installer is saved to a temporary file with a unique
                #   name so that a parallel run doesn't corrupt it;
                # - the installer auto-detects the CPU architecture on its own
                installer="$(mktemp syft.install.XXXXXX.sh)"
                if curl -fsSL -o "${installer}" "${SYFT_URL}" \
    ...
    sh '''
        ...
        install_oras() {
            ...
            for i in 1 2 3; do
                ...
                archive="$(mktemp oras.XXXXXX.tar.gz)"
                if curl -fsSL -o "${archive}" "${ORAS_URL}" \
                    && sudo tar -xzf "${archive}" -C /usr/local/bin oras
                then
                    rm -f "${archive}"
                    break
                fi
...
def installAwsCli() {
    sh '''
        ...
        install_aws_cli() {
            ...
            for i in 1 2 3; do
                # See the note on a temporary file in `installSyft`
                tmp_dir="$(mktemp -d aws.XXXXXX)"
                if curl -fsSL -o "${tmp_dir}/awscli.zip" "${AWS_CLI_URL}" \
                    && unzip -q -o "${tmp_dir}/awscli.zip" -d "${tmp_dir}" \
 ...
 */
abstract class DockerRegistry implements Serializable {
    CpsScript cpsScript
    String repoPath
    String credId
     ...
     * parameter is for.
     */
    DockerRegistry(CpsScript cpsScript, String repoPath, String credId) {
    ...
    /**
     * Logs in into the Docker registry account with the Docker CLI.
     *
     * Call the function before using Docker CLI (e.g. `docker push`) against
     * the registry.
     */
    abstract void dockerLogin()

    /**
     * Logs in into the Docker registry account with the Oras CLI.
     *
     * Call the function before using Oras CLI (e.g. `oras attach`) against
     * the registry. The function also enables using the Docker CLI
     * (e.g. `docker push`) against the registry.
     */
    abstract void orasLogin()
    ...
    void attachSboms(Version v) {
        ...
        cpsScript.sh """
            ...
            inspectionResult="\$(docker buildx imagetools inspect --raw \${ref})"

            for arch in 'amd64' 'arm64'; do
                digest=\$(echo "\${inspectionResult}" \\
                    | jq -r ".manifests[] \\
                             | select(.platform.architecture==\\\"\${arch}\\\") \\
                             | .digest")
                [ -n "\$digest" ] || abort "failed to resolve \${arch} digest"

                sbomFile="percona-server-mongodb-${v}-\${arch}.cdx.json"
                purl="pkg:oci/percona-server-mongodb@\${digest}?repository_url=${this.repoPath}"
...
class DockerHub extends DockerRegistry {
    DockerHub(CpsScript script, String repoPath) {
    ...
    void orasLogin() {
        ...
        cpsScript.withCredentials([cpsScript.usernamePassword(credentialsId: this.credId,
                                                              ...
                                                              usernameVariable: 'USER')]) {
            cpsScript.sh '''
                echo "${PASS}" | oras login -u "${USER}" --password-stdin docker.io
            '''
        }
    }
...
class AwsEcr extends DockerRegistry {
    ...
    AwsEcr(CpsScript script) {
        super(
            script,
            'public.ecr.aws/e7j3v3n0/psmdb-build',
            '8468e4e0-5371-4741-a9bb-7c143140acea'
        )
    ...
    void dockerLogin() {
        ...
        cpsScript.withCredentials([cpsScript.aws(credentialsId: this.credId,
 ...
 */
DockerRegistry createDockerRegistry(CpsScript script, String imageType) {
    switch (imageType) {
        case 'experimental':
...
def buildImage(Version v,
               ...
               String imageType,
               String debug) {
    String dockerfile = 'Dockerfile' + (arch == 'arm64' ? '.aarch64' : '')
    sh """
        # Clone the latest revision of the only necessary directory
        git clone --no-checkout --depth 1 'https://github.com/percona/percona-docker'
        cd percona-docker/
        git sparse-checkout init --cone
        git sparse-checkout set percona-server-mongodb-${v.feature}
        git checkout
        cd percona-server-mongodb-${v.feature}

        sed -E \\
            -e "s/ENV PSMDB_VERSION (.+)/ENV PSMDB_VERSION ${v}/" \\
            -e "s/ENV PSMDB_REPO (.+)/ENV PSMDB_REPO ${imageType}/" \\
            -e "/^ENV IMAGE_TYPE/d" \\
            -e "/^ENV PSMDB_REPO/a ENV IMAGE_TYPE ${imageType}" \\
            -i "${dockerfile}"

        docker buildx build --provenance=false --sbom=false --load \\
            -f "${dockerfile}" -t percona-server-mongodb:local-${arch} .
 ...
 */
def createSbom(Version v, String arch) {
    installJq()
    installSyft()
    ...
    sh """
        ...
        count=\$(jq '.components | length' "${sbomFile}")
        [ "\${count}" -lt 10 ] && abort "${arch} SBOM has only \${count} components"
        echo "${arch} SBOM: ${sbomFile} (\${count} components)"
    """
...
def testWithPbmStage(String psmdbVersion,
    ...
    //   Save them to the `pbm_heads.txt` file.
    // - For each line:
    //   - remove the `<40_char_hash><some_spaces>refs/heads/` part, keeping
    //     only `<branch_name>`;
    //   - if `<branch_name>` starts with "release-", print it to standard
    //     output; otherwise discard it.
    // - Sort the printed branch names by version number.
    // - Take only the last one, e.g. that with the highest version.
    //
    // Note. The script could have been written in a more concise form just by
    // redirecting the `git ls-remote` output to `sed`. But such an approach
    // would swallow `git ls-remote` errors.
    def pbmLatestReleaseBranch = sh(
        returnStdout: true,
        script: '''
            abort() {
                printf "Error: %s\n" "${1:-unknown error}" >&2
            ...
            pbmRepo=https://github.com/percona/percona-backup-mongodb.git
            git ls-remote --heads "\${pbmRepo}" > pbm_heads.txt || abort '`git ls-remote` failed'
            pbmBranch=$(sed -n -e 's|.*refs/heads/||g' -e '/^release-.*/p' pbm_heads.txt \
                | sort --version-sort | tail -1)

            rm -f pbm_heads.txt
            [ -n "$pbmBranch" ] || abort "failed to resolve the latest PBM release branch"
    ...
    build \
        ...
        parameters: [
            ...
            string(name: 'PBM_BRANCH', value: pbmLatestReleaseBranch),
            string(name: 'PSMDB', value: psmdbImage),
            string(name: 'TESTING_BRANCH', value: "pbm-${pbmLatestReleaseBranch}"),
...
pipeline {
    ...
    parameters {
        ...
        choice(name: 'IMAGE_TYPE',
               ...
               description: "Type of the Docker images to create. The pipeline pushes "
        ...
        string(name: 'PSMDB_VERSION', defaultValue: '8.0.26-11', description: 'PSMDB version')
            ...
            parallel {
                stage('amd64') {
                    ...
                    post {
                        always {
                            sh '''
                                images=$(sudo docker image ls -q  | sort -u)
                                [ -z "$images" ] ||  sudo docker image rm -f $images
                            '''
                            deleteDir()
                        }
                    }
    ...
    post {
        ...
        failure {
            script {
                ...
                slackNotify("#mongodb_autofeed", "#FF0000", msg)
            }
        }
    }
}
