/**
 * pxc9xBuildMatrix.groovy - PXC 9.x build matrix shared library
 *
 * Runs parallel build stages for Percona XtraDB Cluster 9.x packages.
 * Handles agent label selection, FIPS mode, and EXPERIMENTALMODE gating.
 *
 * Usage:
 *   pxc9xBuildMatrix(
 *       cloud:           params.CLOUD,
...
def call(Map args = [:]) {
    ...
    def experimentalMode = args.get('experimentalMode', 'NO')
    def onlyStages       = args.get('onlyStages', [])

    // Each entry: name, image, arch, buildType, flags, fipsFlags (null = always use plain flags),
    //             skipInFips (true = skip entire stage when FIPS)
    def stages = [
        // ---- RPM stages ----
        [name: 'Centos 8',                 image: 'centos:8',         arch: 'x64',     buildType: 'rpm',
         flags: '--build_rpm=1',           fipsFlags: null,           skipInFips: true],
        ...
        [name: 'Oracle Linux 9',           image: 'oraclelinux:9',    arch: 'x64',     buildType: 'rpm',
         flags: '--build_rpm=1',           fipsFlags: '--build_rpm=1 --enable_fipsmode=1', skipInFips: false],
        ...
        [name: 'Amazon Linux 2023',        image: 'amazonlinux:2023', arch: 'x64',     buildType: 'rpm',
         flags: '--build_rpm=1',           fipsFlags: null,           skipInFips: false],
        ...
        [name: 'Ubuntu Jammy(22.04)',      image: 'ubuntu:jammy',     arch: 'x64',     buildType: 'deb',
         flags: '--build_deb=1',           fipsFlags: '--build_deb=1 --enable_fipsmode=1', skipInFips: false],
        ...
        [name: 'Centos 8 tarball',         image: 'centos:8',         arch: 'x64',     buildType: 'tarball',
         flags: '--build_tarball=1',       fipsFlags: null,           skipInFips: true],
        [name: 'Oracle Linux 9 tarball',   image: 'oraclelinux:9',    arch: 'x64',     buildType: 'tarball',
         flags: '--build_tarball=1',       fipsFlags: '--build_tarball=1 --enable_fipsmode=1', skipInFips: false],
        [name: 'Ubuntu Jammy(22.04) tarball', image: 'ubuntu:jammy',  arch: 'x64',     buildType: 'tarball',
         flags: '--build_tarball=1',       fipsFlags: '--build_tarball=1 --enable_fipsmode=1', skipInFips: false],
        [name: 'Ubuntu Noble(24.04) tarball', image: 'ubuntu:noble',  arch: 'x64',     buildType: 'tarball',
         flags: '--build_tarball=1',       fipsFlags: '--build_tarball=1 --enable_fipsmode=1', skipInFips: false],
    ]
