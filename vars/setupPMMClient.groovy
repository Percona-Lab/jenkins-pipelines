                ...
                if [[ \$PMM_VERSION == pmm1 ]]; then
                    ...
                    if [[ \$CLIENT_INSTANCE == yes ]]; then
                        if [[ \$ENABLE_PULL_MODE == yes ]]; then
                          sudo pmm-agent setup --config-file=`pwd`/pmm2-client/config/pmm-agent.yaml --server-address=\$SERVER_IP:443 --server-insecure-tls --server-username=admin --server-password=\$ADMIN_PASSWORD --metrics-mode=pull \$IP
                        else
                            sudo pmm-agent setup --config-file=`pwd`/pmm2-client/config/pmm-agent.yaml --server-address=\$SERVER_IP:443 --server-insecure-tls --server-username=admin --server-password=\$ADMIN_PASSWORD \$IP
                        fi
                    else
                        sudo pmm-agent setup --config-file=`pwd`/pmm2-client/config/pmm-agent.yaml --server-address=\$IP:443 --server-insecure-tls --server-username=admin --server-password=\$ADMIN_PASSWORD \$IP
                    fi
                    sleep 10
                    JENKINS_NODE_COOKIE=dontKillMe nohup bash -c ' sudo pmm-agent --config-file=`pwd`/pmm2-client/config/pmm-agent.yaml > pmm-agent.log 2>&1 &'
                    sleep 10
                    cat pmm-agent.log
                    pmm-admin status
                fi
