...
def templates = [
       /* new HetznerServerTemplate("ubuntu20-cx21", "java", "name=ubuntu20-docker", "fsn1", "cx21"), */
        ...
        new HetznerServerTemplate("deb12-x64",     labelMap['deb12-x64'],     imageMap['deb12-x64'],     "fsn1", "cx51"),
        new HetznerServerTemplate("deb12-aarch64", labelMap['deb12-aarch64'], imageMap['deb12-aarch64'], "fsn1", "cax31"),
        new HetznerServerTemplate("launcher-x64",  labelMap['launcher-x64'],  imageMap['launcher-x64'],  "fsn1", "cx21")
]

templates.each { it -> it.setConnector(new SshConnectorAsRoot("hz.ps3.cd"))
...
def cloud = new HetznerCloud(cloudName, "ps3.cd.hz", "10", templates)
