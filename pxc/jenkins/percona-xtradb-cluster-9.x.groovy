...
boolean shouldRunStage(String stageName) {
    ...
    return params.BUILD_STAGES.split(',').any { it.trim() == stageName }
}

boolean shouldRunTarball() {
    if (!params.BUILD_STAGES) return true
    return params.BUILD_STAGES.split(',').any { it.trim().toLowerCase().contains('tarball') }
}
        ...
        stage('Compute build flags') {
            steps {
                script {
                    ...
                    env.RUN_CENTOS8       = s('Centos 8').toString()
                    env.RUN_CENTOS8_ARM   = s('Centos 8 ARM').toString()
                    ...
                    env.RUN_RESOLUTE      = s('Ubuntu Resolute(26.04)').toString()
                    env.RUN_RESOLUTE_ARM  = s('Ubuntu Resolute(26.04) ARM').toString()
        ...
        stage('Build PXC Tarballs') {
            parallel {
                ...
                stage('Ubuntu Noble(24.04) tarball') {
                    when {
                        environment name: 'RUN_NOBLE_TAR', value: 'true'
                    }
                    agent {
                        label params.CLOUD == 'Hetzner' ? 'docker-x64' : 'docker-32gb'
                    }
