        ...
        stage('Get release branches') {
            steps {
                withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', accessKeyVariable: 'AWS_ACCESS_KEY_ID', credentialsId: 'AWS_STASH', secretKeyVariable: 'AWS_SECRET_ACCESS_KEY']]) {
                    sh """
                        EC=0
                        aws s3 ls s3://percona-jenkins-artifactory/percona-xtradb-cluster/branch_commit_id_97.properties || EC=\$?
			...
			if [ \${EC} = 1 ]; then
			  ...
			  echo "BRANCH_NAME=\${BRANCH_NAME}" > branch_commit_id_97.properties
			  echo "COMMIT_ID=\${COMMIT_ID}" >> branch_commit_id_97.properties

			  aws s3 cp ./branch_commit_id_97.properties s3://percona-jenkins-artifactory/percona-xtradb-cluster/
			  ...
			  echo "BRANCH_NAME=\${LATEST_BRANCH_NAME}" > branch_commit_id_97.properties
			  echo "COMMIT_ID=\${LATEST_COMMIT_ID}" >> branch_commit_id_97.properties
                          aws s3 cp ./branch_commit_id_97.properties s3://percona-jenkins-artifactory/percona-xtradb-cluster/
                        fi
                    """
                }
                script {
                    ...
                    BRANCH_NAME = sh(returnStdout: true, script: "source ./branch_commit_id_97.properties; echo \${BRANCH_NAME}").trim()
                    COMMIT_ID = sh(returnStdout: true, script: "source ./branch_commit_id_97.properties; echo \${COMMIT_ID}").trim()
                    VERSION = sh(returnStdout: true, script: "source ./branch_commit_id_97.properties; echo \${BRANCH_NAME} | cut -d - -f 2 ").trim()
                    RELEASE = sh(returnStdout: true, script: "source ./branch_commit_id_97.properties; echo \${BRANCH_NAME} | cut -d - -f 3 ").trim()
                }
