...
void buildStage(String DOCKER_OS, String STAGE_PARAM) {
    sh """
        set -o xtrace
        mkdir test
        wget \$(echo ${GIT_REPO} | sed -re 's|github.com|raw.githubusercontent.com|; s|\\.git\$||')/${GIT_BRANCH}/postgis/postgis33_builder.sh -O postgis_builder.sh
        ...
        docker run -u root -v \${build_dir}:\${build_dir} ${DOCKER_OS} sh -c "
            set -o xtrace
            cd \${build_dir}
            bash -x ./postgis_builder.sh --builddir=\${build_dir}/test --install_deps=1 --repo_component=${COMPONENT}
            bash -x ./postgis_builder.sh --builddir=\${build_dir}/test --repo_component=${COMPONENT} ${STAGE_PARAM}"
    """
}
