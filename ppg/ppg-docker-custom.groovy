...
pipeline {
        ...
        stage ('Build image') {
            steps {
                sh """
                    ...
                    docker build --platform=linux/amd64 --no-cache --provenance=false -t percona-distribution-postgresql-custom:${env.IMAGE_VER}-v3 -f Dockerfile-postgis .
                    """
            }
        }
                ...
                sh """
                    ...
                    if [ ${params.PPG_REPO} = "release" ]; then
                        /usr/local/bin/trivy -q image --format template --template @junit.tpl  -o trivy-hight-junit.xml \
                                         --timeout 10m0s --ignore-unfixed --exit-code 1 --severity HIGH,CRITICAL percona-distribution-postgresql-custom:${env.IMAGE_VER}-v3
                    else
                        /usr/local/bin/trivy -q image --format template --template @junit.tpl  -o trivy-hight-junit.xml \
                                         --timeout 10m0s --ignore-unfixed --exit-code 0 --severity HIGH,CRITICAL percona-distribution-postgresql-custom:${env.IMAGE_VER}-v3
                    fi
               """
            }
            post {
    ...
    post {
        ...
        success {
            slackNotify("#releases-ci", "#00FF00", "[${JOB_NAME}]: Building of PPG ${env.IMAGE_VER}-v3 repo ${PPG_REPO} succeed")
        }
        unstable {
            slackNotify("#releases-ci", "#F6F930", "[${JOB_NAME}]: Building of PPG ${env.IMAGE_VER}-v3 repo ${PPG_REPO} unstable - [${BUILD_URL}testReport/]")
        }
        failure {
            slackNotify("#releases-ci", "#FF0000", "[${JOB_NAME}]: Building of PPG ${env.IMAGE_VER}-v3 repo ${PPG_REPO} failed - [${BUILD_URL}]")
        }
    }
}
