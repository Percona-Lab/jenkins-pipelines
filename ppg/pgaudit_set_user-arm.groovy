...
void buildStage(String DOCKER_OS, String STAGE_PARAM) {
    sh """
        set -o xtrace
        mkdir test
        wget \$(echo ${GIT_REPO} | sed -re 's|github.com|raw.githubusercontent.com|; s|\\.git\$||')/${GIT_BRANCH}/pgaudit_set_user/pgaudit_builder.sh -O builder.sh
        pwd -P
        ls -laR
...
pipeline {
    ...
    parameters {
        ...
        string(
            defaultValue: '4.0.1',
            description: 'Tag/Branch for pg_audit-set-user',
            name: 'PG_BRANCH')
        string(
            defaultValue: '16.1',
            description: 'Tag/Branch for pg_audit-set-user packaging repository',
            name: 'GIT_BRANCH')
        string(
            defaultValue: '1',
