...
void buildStage(String DOCKER_IMAGE, String STAGE_PARAM) {
    sh """
        set -o xtrace
        mkdir -p test
        wget \$(echo ${GIT_REPO} | sed -re 's|github.com|raw.githubusercontent.com|; s|\\.git\$||')/master/gdal385/builder.sh -O builder.sh
        pwd -P
...
pipeline {
    ...
    parameters {
        ...
        string(name: 'GIT_REPO', defaultValue: 'https://github.com/percona/postgres-packaging.git', description: 'URL for gdal385 repository')
    ...
    stages {
        stage('Build SRPM (generic)') {
            ...
            steps {
                ...
                sh '''
                    REPO_UPLOAD_PATH=$(grep "UPLOAD" test/gdal385.properties | cut -d = -f 2 | sed "s:$:${BUILD_NUMBER}:")
                    AWS_STASH_PATH=$(echo ${REPO_UPLOAD_PATH} | sed "s:UPLOAD/experimental/::")
                    echo ${REPO_UPLOAD_PATH} > uploadPath
                    echo ${AWS_STASH_PATH} > awsUploadPath
                    cat test/gdal385.properties
                    cat uploadPath
                '''
                script {
        ...
        stage('Build RPMs in Parallel') {
            parallel {
                ...
                stage('Build Oracle Linux 8 aarch64') {
                    agent {
