...
void buildStage(String DOCKER_OS, String STAGE_PARAM) {
    sh """
        set -o xtrace
        mkdir test
        wget \$(echo ${GIT_REPO} | sed -re 's|github.com|raw.githubusercontent.com|; s|\\.git\$||')/${GIT_BRANCH}/pgbackrest/pgbackrest_builder.sh -O builder.sh
        ...
        docker run -u root -v \${build_dir}:\${build_dir} ${DOCKER_OS} sh -c "
            set -o xtrace
            cd \${build_dir}
            bash -x ./builder.sh --builddir=\${build_dir}/test --install_deps=1 --repo_component=${COMPONENT}
...
pipeline {
    ...
    parameters {
        ...
        string(
            defaultValue: '17.6',
            description: 'Tag/Branch for pg_BACKREST packaging repository',
            name: 'GIT_BRANCH')
        string(
    ...
    stages {
        stage('Create PG_BACKREST source tarball') {
            steps {
                slackNotify("#releases-ci", "#00FF00", "[${JOB_NAME}]: starting build for ${GIT_BRANCH} - [${BUILD_URL}]")
                cleanUpWS()
                buildStage("oraclelinux:8", "--get_sources=1")
                sh '''
                   REPO_UPLOAD_PATH=$(grep "UPLOAD" test/pgbackrest.properties | cut -d = -f 2 | sed "s:$:${BUILD_NUMBER}:")
                   AWS_STASH_PATH=$(echo ${REPO_UPLOAD_PATH} | sed  "s:UPLOAD/experimental/::")
                   echo ${REPO_UPLOAD_PATH} > uploadPath
                   echo ${AWS_STASH_PATH} > awsUploadPath
                   cat test/pgbackrest.properties
                   cat uploadPath
                '''
                script {
        ...
        stage('Build PG_BACKREST generic source packages') {
            parallel {
                stage('Build PG_BACKREST generic source rpm') {
                    agent {
                        label params.CLOUD == 'Hetzner' ? 'docker-x64-min' : 'docker'
                    }
                    steps {
                ...
                stage('Build PG_BACKREST generic source deb') {
                    agent {
                        label params.CLOUD == 'Hetzner' ? 'docker-x64-min' : 'docker'
                    }
                    steps {
        ...
        stage('Build PG_BACKREST RPMs/DEBs/Binary tarballs') {
            parallel {
