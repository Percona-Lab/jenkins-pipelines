        ...
        stage ('Build image') {
            steps {
                sh """
                    ...
                    docker build ---platform=linux/amd64 --no-cache . -t percona-distribution-postgresql:\$MAJ_VER 
                    docker build --platform=linux/amd64 --no-cache . -t percona-distribution-postgresql-with-postgis:\$MAJ_VER -f Dockerfile-postgis
                    """
            }
        }
