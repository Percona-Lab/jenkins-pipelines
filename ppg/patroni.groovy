...
void buildStage(String DOCKER_OS, String STAGE_PARAM) {
    sh """
        set -o xtrace
        mkdir test
        wget \$(echo ${GIT_REPO} | sed -re 's|github.com|raw.githubusercontent.com|; s|\\.git\$||')/${GIT_BRANCH}/patroni/patroni_builder.sh -O builder.sh
...
pipeline {
    ...
    parameters {
        ...
        string(
            defaultValue: '17.6',
            description: 'Tag/Branch for patroni packaging repository',
            name: 'GIT_BRANCH')
        string(
    ...
    stages {
        stage('Create PATRONI source tarball') {
            steps {
                ...
                sh '''
                   REPO_UPLOAD_PATH=$(grep "UPLOAD" test/patroni.properties | cut -d = -f 2 | sed "s:$:${BUILD_NUMBER}:")
                   AWS_STASH_PATH=$(echo ${REPO_UPLOAD_PATH} | sed  "s:UPLOAD/experimental/::")
                   echo ${REPO_UPLOAD_PATH} > uploadPath
                   echo ${AWS_STASH_PATH} > awsUploadPath
                   cat test/patroni.properties
                   cat uploadPath
                '''
                script {
        ...
        stage('Build PATRONI generic source packages') {
            parallel {
                stage('Build PATRONI generic source rpm') {
                    agent {
                        label params.CLOUD == 'Hetzner' ? 'docker-x64-min' : 'docker'
                    }
                    steps {
