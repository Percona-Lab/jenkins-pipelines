        ...
        stage ('Build image') {
            ...
            steps {
                sh """
                    ...
                    docker buildx build --platform=linux/arm64 --no-cache --provenance=false --sbom=false --output type=docker,oci-mediatypes=true -t percona-pgbackrest .
                    """
            }
        }
...
EOS
                        chmod +x /tmp/push_oci.sh
                         /tmp/push_oci.sh percona-pgbackrest perconalab/percona-pgbackrest:${env.IMAGE_VER}-arm64
                         docker buildx imagetools create -t perconalab/percona-pgbackrest:\$MAJ_VER-arm64 perconalab/percona-pgbackrest:${env.IMAGE_VER}-arm64
                         docker buildx imagetools create -t perconalab/percona-pgbackrest:${env.IMAGE_VER} \
                            perconalab/percona-pgbackrest:${env.IMAGE_VER}-amd64 \
                            perconalab/percona-pgbackrest:${env.IMAGE_VER}-arm64
                         docker buildx imagetools inspect perconalab/percona-pgbackrest:${env.IMAGE_VER}
                         docker buildx imagetools create -t perconalab/percona-pgbackrest:\$MAJ_VER \
                            perconalab/percona-pgbackrest:\$MAJ_VER-amd64 \
                            perconalab/percona-pgbackrest:\$MAJ_VER-arm64
                         docker buildx imagetools inspect perconalab/percona-pgbackrest:\$MAJ_VER
                         if [ ${params.LATEST} = "yes" ]; then
                            docker buildx imagetools create -t perconalab/percona-pgbackrest:latest \
                               perconalab/percona-pgbackrest:\$MAJ_VER-amd64 \
                               perconalab/percona-pgbackrest:\$MAJ_VER-arm64
                            docker buildx imagetools inspect perconalab/percona-pgbackrest:latest
                         fi
                ...
                withCredentials([usernamePassword(credentialsId: 'hub.docker.com', passwordVariable: 'PASS', usernameVariable: 'USER')]) {
                     sh """
                         ...
                         docker buildx imagetools create \\
                             -t percona/percona-pgbackrest:${env.IMAGE_VER}-arm64 \\
                             perconalab/percona-pgbackrest:${env.IMAGE_VER}-arm64
                         docker buildx imagetools create \\
                             -t percona/percona-pgbackrest:\$MAJ_VER-arm64 \\
                             perconalab/percona-pgbackrest:\$MAJ_VER-arm64
                         docker buildx imagetools create \\
                             -t percona/percona-pgbackrest:${env.IMAGE_VER} \\
                             perconalab/percona-pgbackrest:${env.IMAGE_VER}-amd64 \\
                             perconalab/percona-pgbackrest:${env.IMAGE_VER}-arm64
                         docker buildx imagetools create \\
                             -t percona/percona-pgbackrest:\$MAJ_VER \\
                             perconalab/percona-pgbackrest:\$MAJ_VER-amd64 \\
                             perconalab/percona-pgbackrest:\$MAJ_VER-arm64
                         if [ ${params.LATEST} = "yes" ]; then
                             docker buildx imagetools create \\
                                 -t percona/percona-pgbackrest:latest \\
                                 perconalab/percona-pgbackrest:\$MAJ_VER-amd64 \\
                                 perconalab/percona-pgbackrest:\$MAJ_VER-arm64
                         fi
                     """
