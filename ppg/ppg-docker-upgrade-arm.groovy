        ...
        stage ('Build image') {
            steps {
                sh """
                    ...
                    cd percona-docker/postgresql-containers
                    sed -i "s|ppg-\\\${PG_MAJOR} release|ppg-\\\${PG_MAJOR} ${params.PPG_REPO}|g" build/upgrade/Dockerfile
                    sed -i "s|ppg-\\\${pg_version} release|ppg-\\\${pg_version} ${params.PPG_REPO}|g" build/upgrade/Dockerfile
                    sed -i "s|x86_64|aarch64|g" build/upgrade/Dockerfile
                    sed -i "s|ppg-\\\${PG_MAJOR} release|ppg-\\\${PG_MAJOR} ${params.PPG_REPO}|g" build/upgrade/Dockerfile-custom
                    sed -i "s|ppg-\\\${pg_version} release|ppg-\\\${pg_version} ${params.PPG_REPO}|g" build/upgrade/Dockerfile-custom
                    ...
                    if [ "${params.IMAGE_TYPE}" = "both" ] || [ "${params.IMAGE_TYPE}" = "custom" ]; then
                        docker build --platform=linux/arm64 --no-cache --provenance=false \\
                            ...
                            -f build/upgrade/Dockerfile-custom .
                    fi
                    """
            }
        }
