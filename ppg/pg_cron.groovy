...
void buildStage(String DOCKER_OS, String STAGE_PARAM) {
    sh """
        set -o xtrace
        mkdir test
        wget \$(echo ${GIT_REPO} | sed -re 's|github.com|raw.githubusercontent.com|; s|\\.git\$||')/${GIT_BRANCH}/pg_cron/pg_cron_builder.sh -O builder.sh
        pwd -P
        ls -laR
...
pipeline {
    ...
    parameters {
        ...
        string(
            ...
            description: 'URL for pg_cron repository',
            name: 'GIT_REPO')
        string(
            defaultValue: '16.1',
            description: 'Tag/Branch for pg_cron repository',
            name: 'PG_BRANCH')
        string(
            defaultValue: '16.1',
            description: 'Tag/Branch for pg_cron packaging repository',
            name: 'GIT_BRANCH')
        string(
            defaultValue: '1',
    ...
    stages {
        stage('Create pg_cron source tarball') {
            steps {
                slackNotify("#releases-ci", "#00FF00", "[${JOB_NAME}]: starting build for ${GIT_BRANCH} - [${BUILD_URL}]")
                cleanUpWS()
                buildStage("oraclelinux:8", "--get_sources=1")
                sh '''
                   REPO_UPLOAD_PATH=$(grep "UPLOAD" test/pg_cron.properties | cut -d = -f 2 | sed "s:$:${BUILD_NUMBER}:")
                   AWS_STASH_PATH=$(echo ${REPO_UPLOAD_PATH} | sed  "s:UPLOAD/experimental/::")
                   echo ${REPO_UPLOAD_PATH} > uploadPath
                   echo ${AWS_STASH_PATH} > awsUploadPath
                   cat test/pg_cron.properties
                   cat uploadPath
                '''
                script {
        ...
        stage('Build pg_cron generic source packages') {
            parallel {
                stage('Build pg_cron generic source rpm') {
                    agent {
                        label params.CLOUD == 'Hetzner' ? 'docker-x64-min' : 'docker'
                    }
                    steps {
                ...
                stage('Build pg_cron generic source deb') {
                    agent {
                        label params.CLOUD == 'Hetzner' ? 'docker-x64-min' : 'docker'
                    }
                    steps {
        ...
        stage('Build pg_cron RPMs/DEBs/Binary tarballs') {
            parallel {
