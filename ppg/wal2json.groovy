...
void buildStage(String DOCKER_OS, String STAGE_PARAM) {
    sh """
        set -o xtrace
        mkdir test
        wget \$(echo ${GIT_REPO} | sed -re 's|github.com|raw.githubusercontent.com|; s|\\.git\$||')/${GIT_BRANCH}/wal2json/wal2json_builder.sh -O builder.sh
...
pipeline {
    ...
    parameters {
        ...
        string(
            ...
            description: 'URL for wal2json repository',
            name: 'GIT_REPO')
        string(
            defaultValue: 'ver_1.4.4',
            description: 'Tag/Branch for wal2json repository',
            name: 'PG_BRANCH')
        string(
            defaultValue: '16.0',
            description: 'Tag/Branch for wal2json packaging repository',
            name: 'GIT_BRANCH')
        string(
            defaultValue: '1',
    ...
    stages {
        stage('Create wal2json source tarball') {
            steps {
                slackNotify("#releases-ci", "#00FF00", "[${JOB_NAME}]: starting build for ${GIT_BRANCH} - [${BUILD_URL}]")
                cleanUpWS()
                buildStage("oraclelinux:8", "--get_sources=1")
                sh '''
                   REPO_UPLOAD_PATH=$(grep "UPLOAD" test/wal2json.properties | cut -d = -f 2 | sed "s:$:${BUILD_NUMBER}:")
                   AWS_STASH_PATH=$(echo ${REPO_UPLOAD_PATH} | sed  "s:UPLOAD/experimental/::")
                   echo ${REPO_UPLOAD_PATH} > uploadPath
                   echo ${AWS_STASH_PATH} > awsUploadPath
                   cat test/wal2json.properties
                   cat uploadPath
                '''
                script {
        ...
        stage('Build wal2json generic source packages') {
            parallel {
                ...
                stage('Build wal2json generic source deb') {
                    agent {
                        label params.CLOUD == 'Hetzner' ? 'docker-x64-min' : 'docker'
                    }
                    steps {
        ...
        stage('Build wal2json RPMs/DEBs/Binary tarballs') {
            parallel {
