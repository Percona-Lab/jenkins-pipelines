            ...
            steps {
                script {
                    if (params.WITH_POSTGIS) {
                        currentBuild.displayName = "${env.BUILD_NUMBER}-docker-with-postgis-${env.SERVER_VERSION}-${env.PLATFORM}-${env.REPOSITORY}-${env.DOCKER_TAG}"
                    } else {
                        currentBuild.displayName = "${env.BUILD_NUMBER}-docker-${env.SERVER_VERSION}-${env.PLATFORM}-${env.REPOSITORY}-${env.DOCKER_TAG}"
                    }
                }
            }
        }
