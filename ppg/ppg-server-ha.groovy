...
void buildStage(String DOCKER_OS, String STAGE_PARAM) {
    sh """
        ...
        docker run -u root -v \${build_dir}:\${build_dir} ${DOCKER_OS} sh -c "
            set -o xtrace
            cd \${build_dir}
            bash -x ./ppg-server-ha_builder.sh --builddir=\${build_dir}/test --install_deps=1 --repo_component=${COMPONENT}
            bash -x ./ppg-server-ha_builder.sh --builddir=\${build_dir}/test --repo_component=${COMPONENT} ${STAGE_PARAM}"
    """
}
