...
pipeline {
    ...
    stages {
        stage ('Run PCSM functional tests') {
            steps {
                build job: 'hetzner-pcsm-functional-tests-full', propagate: false, parameters: [ string(name: 'CLOUD', value: params.CLOUD), string(name: 'PCSM_BRANCH', value: 'release-' + params.PCSM_VERSION )]
            }
        }
