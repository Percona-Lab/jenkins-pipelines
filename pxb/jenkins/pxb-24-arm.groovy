...
void buildStage(String DOCKER_OS, String STAGE_PARAM) {
    sh """
        ...
        docker run -u root -v \${build_dir}:\${build_dir} ${DOCKER_OS} sh -c "
            ...
            bash -x ./percona-xtrabackup-2.4_builder.sh --builddir=\${build_dir}/test --repo=${GIT_REPO} --branch=${BRANCH} --pxb_repo=${PXB_REPO} --rpm_release=${RPM_RELEASE} ${STAGE_PARAM}"
    """
}
        ...
        stage('Create PXB source tarball') {
            ...
            steps {
                ...
                script {
                    ...
                    XB_VERSION_MAJOR = sh(returnStdout: true, script: "grep 'XB_VERSION_MAJOR' ./test/percona-xtrabackup-2.4.properties | cut -d = -f 2 ").trim()
                    XB_VERSION_MINOR = sh(returnStdout: true, script: "grep 'XB_VERSION_MINOR' ./test/percona-xtrabackup-2.4.properties | cut -d = -f 2 ").trim()
                    XB_VERSION_PATCH = sh(returnStdout: true, script: "grep 'XB_VERSION_PATCH' ./test/percona-xtrabackup-2.4.properties | cut -d = -f 2 ").trim()
                    XB_VERSION_EXTRA = sh(returnStdout: true, script: "grep 'XB_VERSION_EXTRA' ./test/percona-xtrabackup-2.4.properties | cut -d = -f 2 | sed 's/-//g'").trim()
                }
