...
pipeline {
  ...
  parameters {
    ...
    string(
      ...
      description: 'DOCKER_OS used to build the NEW PXB binary for the Debian-family test host (debian-12). Must be a valid DOCKER_OS choice of the target compile pipeline (see PXB_BUILD_DOCKER_OS_RHEL for valid values).'
    )
    choice(
      name: 'PXB_BUILD_TYPE',
    ...
    stage('Build PXB from branch') {
      ...
      steps {
        script {
          ...
          // a released download (handled by the molecule role), so only NEW is built here.
          def buildOne = { dockerOS ->
            echo "Building unreleased NEW PXB from ${params.PXB_GIT_REPO} branch '${params.PXB_BRANCH}' via ${compileJob} (${dockerOS}, ${params.PXB_BUILD_TYPE}); OLD_PXB_VERSION stays a released download"
          ...
          // Build the RedHat (oracle-9) and Debian (debian-12) NEW PXB binaries in parallel.
          def urls = [:]
          parallel(
          ...
          echo "Branch-built NEW PXB tarball (RedHat/oracle-9): ${env.PXB_TARBALL_URL_RHEL}"
          echo "Branch-built NEW PXB tarball (Debian/debian-12): ${env.PXB_TARBALL_URL_DEB}"
