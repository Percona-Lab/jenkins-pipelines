...
pipeline {
    ...
    stages {
        ...
        stage('Start staging') {
            steps {
                runStaging(DOCKER_VERSION, CLIENT_VERSION, '--addclient=ps,1 --pmm2', PMM_QA_GIT_BRANCH, PMM_QA_GIT_COMMIT_HASH, ADMIN_PASSWORD)
                script {
    ...
    post {
        ...
        unstable {
            slackSend channel: '#pmm-notifications', color: '#00FF00', message: "[${JOB_NAME}]: build failed\nok - ${OK}, skip - ${SKIP}, fail - ${FAIL}\ncheck here: ${BUILD_URL}"
        }
        success {
