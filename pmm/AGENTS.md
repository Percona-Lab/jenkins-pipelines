...
Related: [IaC/cdk/](../IaC/cdk/) (Lambda cleanup functions for OpenShift/AWS resources)
...
- **Lambda cleanup functions** exist in `IaC/cdk/` for automated AWS resource cleanup (OpenShift clusters, EC2 instances). See `LambdaOpenShiftCleanup` in us-east-2.
...
Data source: Jenkins CLI (`~/bin/jenkins job pmm`) + codebase scan. Last updated: 2025-12.
...
| pmm3-ha-rosa-test | 33 | 2 | infra | 0 | 0 | man | LOW |
| pmm3-ha-eks | 22 | 2 | infra | 0 | 0 | man | LOW |
| pmm3-ha-eks-cleanup | - | 2 | infra | 0 | 0 | **DISABLED** | HIGH |

Legend: Tier: 0=entry, 1=build, 2=infra, 3=test | Cat: orch=orchestrator, up=upstream-triggered, man=manual

**Operational Alert**: `pmm3-ha-eks-cleanup` is currently DISABLED. EKS clusters created by `pmm3-ha-eks` may not be automatically cleaned up. Monitor AWS costs.
...
- **Feature builds** (on-demand builds for feature branches):
  - Trigger: `pmm3-feature-build-trigger` (Jenkins job exists, script not in repo)
  - Build: `pmm3-feature-build` (Jenkins job exists, script not in repo)
...
- **Migration tests**: `pmm/v3/pmm3-migration-tests.groovy`

### Kubernetes

- **EKS HA infra bring-up** (creates cluster, archives kubeconfig; does not delete on success): `pmm/v3/pmm3-ha-eks.groovy`
- **EKS HA cleanup**: `pmm/v3/pmm3-ha-eks-cleanup.groovy` — **CURRENTLY DISABLED** (was `cron('H 0,12 * * *')`)
- **ROSA HA test** (Red Hat OpenShift on AWS, alternative to EKS): `pmm3-ha-rosa-test` (Jenkins job exists, script not in repo)
- **ROSA HA cleanup**: `pmm3-ha-rosa-cleanup-test` (Jenkins job exists, script not in repo)
...
- `pmm/v3/pmm3-ha-eks-cleanup.groovy`: **DISABLED** (was `cron('H 0,12 * * *')`) — EKS cluster cleanup. See Operational Alert above.
...
- **pmm3-ha-eks-cleanup is DISABLED**: EKS clusters may accumulate. Monitor AWS costs in us-east-2.
- **Scripts missing from repo**: Some Jenkins jobs (`pmm3-feature-build`, `pmm3-ha-rosa-*`) reference scripts not present in this repository. These may be WIP, in a different branch, or misconfigured.
