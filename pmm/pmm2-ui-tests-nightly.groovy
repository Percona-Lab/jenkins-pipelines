        ...
        stage('Setup PMM Client and Kubernetes Cluster') {
            parallel {
                ...
                stage('Start Client Instance - mongo') {
                    steps {
                        runStagingClient(DOCKER_VERSION, CLIENT_VERSION, '--addclient=mo,1 --with-replica --mongomagic --pmm2', 'yes', env.VM_IP, 'mongo-node', ENABLE_PULL_MODE, PXC_VERSION, PS_VERSION, MS_VERSION, PGSQL_VERSION, PDPGSQL_VERSION, MD_VERSION, MO_VERSION, MODB_VERSION, QUERY_SOURCE)
                    }
                }
                stage('Start Client Instance - postgresql') {
                    steps {
                        runStagingClient(DOCKER_VERSION, CLIENT_VERSION, '--addclient=pdpgsql,1 --addclient=pgsql,1 --pmm2', 'yes', env.VM_IP, 'postgres-node', ENABLE_PULL_MODE, PXC_VERSION, PS_VERSION, MS_VERSION, PGSQL_VERSION, PDPGSQL_VERSION, MD_VERSION, MO_VERSION, MODB_VERSION, QUERY_SOURCE)
                    }
                }
            }
        }
