        ...
        stage('Run UI pre-upgrade Tests') {
            ...
            steps {
                sh """
                    ...
                    ./node_modules/.bin/codeceptjs run -c pr.codecept.js --grep '@pre-pmm-portal-upgrade'
                """
            }
        }
        stage('Run UI way Upgrade') {
            ...
            steps {
                sh """
                    ...
                    export PWD=\$(pwd);
                    export CHROMIUM_PATH=/usr/bin/chromium
                    ./node_modules/.bin/codeceptjs run -c pr.codecept.js --grep '@pmm-portal-upgrade'  --override '{ "helpers": { "Playwright": { "getPageTimeout": 60000 }}}'
                """
            }
        }
        ...
        stage('Run UI post-upgrade Tests') {
            ...
            steps {
                sh """
                    ...
                    export CHROMIUM_PATH=/usr/bin/chromium
                    ./node_modules/.bin/codeceptjs run -c pr.codecept.js --grep '@post-pmm-portal-upgrade'
                """
            }
        }
    }
