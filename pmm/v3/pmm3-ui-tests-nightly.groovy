        ...
        stage('Setup PMM Clients') {
            parallel {
                ...
                stage('pdpgsql, pgsql and mysql') {
                    steps {
                        runStagingClient(DOCKER_VERSION, CLIENT_VERSION, '--database pdpgsql=16 --database pgsql=16 --database mysql', 'yes', env.VM_IP, 'postgres-node', ENABLE_PULL_MODE, PXC_VERSION, PS_VERSION, MS_VERSION, PGSQL_VERSION, PDPGSQL_VERSION, MD_VERSION, PSMDB_VERSION, QUERY_SOURCE, ADMIN_PASSWORD)
                    }
                }
            }
        }
