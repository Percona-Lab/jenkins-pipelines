    ...
    node(env.VM_NAME){
        withCredentials([aws(accessKeyVariable: 'AWS_ACCESS_KEY_ID', credentialsId: 'AMI/OVF', secretKeyVariable: 'AWS_SECRET_ACCESS_KEY')]) {
            sh """
                ...
                bash /srv/pmm-qa/pmm-tests/pmm-3-0-bats-tests/pmm-testsuite.sh | tee /tmp/result.tap
            """
        }
    }
...
void fetchAgentLog(String CLIENT_VERSION) {
    withCredentials([sshUserPrivateKey(credentialsId: 'aws-jenkins', keyFileVariable: 'KEY_PATH', passphraseVariable: '', usernameVariable: 'USER')]) {
        sh '''
            ...
            if [[ ${CLIENT_VERSION} == http* ]]; then
                scp -i "${KEY_PATH}" -o ConnectTimeout=1 -o StrictHostKeyChecking=no \
                    ${USER}@${VM_IP}:workspace/aws-staging-start-pmm3/pmm-agent.log \
                    pmm-agent.log
            fi
        '''
    }
...
pipeline {
    ...
    parameters {
        ...
        string(
            ...
            description: 'pmm-client latest version',
            name: 'PMM_VERSION')
    }
    options {
        skipDefaultCheckout()
    ...
    stages {
        ...
        stage('Start staging') {
            steps {
                runStaging(DOCKER_VERSION, CLIENT_VERSION, '--addclient=ps,1 --pmm', PMM_QA_GIT_BRANCH, PMM_QA_GIT_COMMIT_HASH)
                script {
