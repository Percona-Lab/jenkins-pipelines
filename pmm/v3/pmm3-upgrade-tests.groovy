...
def generateVariants(String PMM_UI_GIT_BRANCH, DOCKER_TAG, DOCKER_TAG_UPGRADE, CLIENT_VERSION, CLIENT_REPOSITORY, PMM_SERVER_LATEST, PMM_QA_GIT_BRANCH, QA_INTEGRATION_GIT_BRANCH) {
    def UPGRADE_TAG = DOCKER_TAG_UPGRADE.isEmpty() ? "Test" : DOCKER_TAG_UPGRADE;
    ...
    for (UPGRADE_VARIANT in UPGRADE_VARIANTS) {
        println "Upgrade Variant is: $UPGRADE_VARIANT with variables $PMM_UI_GIT_BRANCH, $DOCKER_TAG, $UPGRADE_TAG, $CLIENT_VERSION, $CLIENT_REPOSITORY, $PMM_SERVER_LATEST, $PMM_QA_GIT_BRANCH, $QA_INTEGRATION_GIT_BRANCH"
        results.put("Run \"$UPGRADE_VARIANT\" upgrade tests", generateStage(PMM_UI_GIT_BRANCH, DOCKER_TAG, UPGRADE_TAG, CLIENT_VERSION, CLIENT_REPOSITORY, PMM_SERVER_LATEST, PMM_QA_GIT_BRANCH, QA_INTEGRATION_GIT_BRANCH, UPGRADE_VARIANT))
    }
}

def generateStage(PMM_UI_GIT_BRANCH, DOCKER_TAG, DOCKER_TAG_UPGRADE, CLIENT_VERSION, CLIENT_REPOSITORY, PMM_SERVER_LATEST, PMM_QA_GIT_BRANCH, QA_INTEGRATION_GIT_BRANCH, LABEL) {
    return {
