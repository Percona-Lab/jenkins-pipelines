        ...
        stage('Run PMM AMI UI tests'){
            parallel {
                stage('Run PMM AMI UI tests EL7'){
                    steps{
                        script {
                            build job: 'pmm-ami-test', parameters: [ string(name: 'AMI_ID', value: env.AMI_ID_EL7) ]
                        }
                    }
                }
                stage('Run PMM AMI UI tests EL9'){
                    steps{
                        script {
                            build job: 'pmm-ami-test', parameters: [ string(name: 'AMI_ID', value: env.AMI_ID) ]
                        }
                    }
                }
            }
