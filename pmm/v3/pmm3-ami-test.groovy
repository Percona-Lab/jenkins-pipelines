...
pipeline {
    ...
    parameters {
        string(
            ...
            description: 'PMM3 Client version',
            name: 'CLIENT_VERSION')
        string(
            defaultValue: '',
    ...
    post {
        success {
            ...
            script {
                if (params.NOTIFY == "true") {
                    slackSend botUser: true, channel: '#pmm-notifications', color: '#00FF00', message: "[${JOB_NAME}]: build finished - https://${env.AMI_INSTANCE_IP}"
                    if (OWNER_SLACK) {
