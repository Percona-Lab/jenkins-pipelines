...
properties([
    ...
    parameters([
        ...
        string(defaultValue: '', description: 'PMM Client tarball link or FB-code', name: 'TARBALL'),
...
timestamps {
    ...
    try {
        ...
        stage('Package tests') {
            parallel([
                'Oracle Linux 8'   : osStage('Oracle Linux 8', 'min-ol-8-arm64', 'rhel'),
                'Oracle Linux 9'   : osStage('Oracle Linux 9', 'min-ol-9-arm64', 'rhel'),
                'Almalinux 10'     : osStage('Almalinux 10', 'min-alma-10-arm64', 'rhel10'),
                'Ubuntu 22.04'     : osStage('Ubuntu 22.04', 'min-jammy-arm64', 'ubuntu'),
                'Ubuntu 24.04'     : osStage('Ubuntu 24.04', 'min-noble-arm64', 'ubuntu'),
                'Ubuntu 26.04'     : osStage('Ubuntu 26.04', 'min-resolute-arm64', 'ubuntu'),
                'Debian 12'        : osStage('Debian 12', 'min-bookworm-arm64', 'debian'),
                'Debian 13'        : osStage('Debian 13', 'min-trixie-arm64', 'trixie'),
            ])
        }
    } finally {
        stage('Teardown') {
