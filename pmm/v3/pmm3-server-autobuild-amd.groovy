            ...
            steps {
                ...
                script {
                    if (params.DESTINATION == "testing") {
                        env.DOCKER_LATEST_TAG     = "${VERSION}-rc${BUILD_NUMBER}-test-amd64"
                        env.DOCKER_RC_TAG         = "${VERSION}-rc-test-amd64"
                    } else {
                        env.DOCKER_LATEST_TAG     = "3-dev-latest-test-amd64"
                    }
                }
        ...
        stage('Build server docker') {
            steps {
                ...
                sh '''
                    ...
                    export DOCKER_TAG=perconalab/pmm-server:$(date -u '+%Y%m%d%H%M')-test-amd64
