...
pipeline {
    environment {
        specName = 'image-configurator'
        repo     = 'Percona-Lab/image-configurator'
    }
    agent {
    ...
    stages {
        stage('Fetch spec files') {
            steps {
                ...
                unstash 'gitCommit'
                sh """
                    mv packaging/${specName}.spec rhel/SPECS/${specName}.spec
                    mv packaging/*                rhel/SOURCES/
    ...
    post {
        ...
        unstable {
            slackSend channel: '@mykola', color: '#00FF00', message: "[${specName}]: build skipped"
            deleteDir()
        }
        failure {
