        ...
        stage('Checkout and prepare') {
            steps {
                slackNotify(
                    "#releases-ci",
                    "#00FF00",
                    "[${JOB_NAME}]: starting Docker image build for Valkey Bundle ${VALKEY_VERSION} - [${BUILD_URL}]"
                )
        ...
        stage('Setup multiarch') {
            steps {
                sh '''
                    ...
                    printf '%s\\n' ':qemu-aarch64:M::\\x7fELF\\x02\\x01\\x01\\x00\\x00\\x00\\x00\\x00\\x00\\x00\\x00\\x00\\x02\\x00\\xb7\\x00:\\xff\\xff\\xff\\xff\\xff\\xff\\xff\\x00\\xff\\xff\\xff\\xff\\xff\\xff\\xff\\xff\\xfe\\xff\\xff\\xff:/usr/local/bin/qemu-aarch64:POCF' \
                    ...
                    echo "arm64 container reports: ${ARCH}"
                    [ "${ARCH}" = "aarch64" ] || {
                        echo "arm64 emulation is not working on the daemon host"
                        exit 1
                    }
            ...
            steps {
                sh '''
                    ...
                    if [ -z "${BASE_IMAGE}" ] || \
                       [ -z "${BASE_TAG}" ] || \
                       [ -z "${BASE_DIGEST}" ]; then
                        echo "Unable to determine pinned base image from Dockerfile.bundle"
                        exit 1
                    fi
    ...
    post {
        ...
        success {
            slackNotify(
                "#releases-ci",
                "#00FF00",
                "[${JOB_NAME}]: Docker images built and pushed for Valkey Bundle ${VALKEY_VERSION} - [${BUILD_URL}]"
            )

            script {
                currentBuild.description =
                    "Valkey Bundle ${VALKEY_VERSION} Docker images pushed to ${IMAGE_NAME}"
            }
        }
        ...
        failure {
            slackNotify(
                "#releases-ci",
                "#FF0000",
                "[${JOB_NAME}]: Docker image build failed for Valkey Bundle ${VALKEY_VERSION} - [${BUILD_URL}]"
            )
        }
