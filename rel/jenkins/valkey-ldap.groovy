...
void buildStage(String DOCKER_OS, String STAGE_PARAM) {
    sh """
        set -o xtrace
        mkdir test
        git clone https://github.com/EvgeniyPatlan/valkey-ldap.git valkey-ldap
        cd valkey-ldap
        ...
        docker run -u root -v \${build_dir}:\${build_dir} ${DOCKER_OS} sh -c "
            set -o xtrace
            cd \${build_dir}
            bash -x ./valkey-ldap/packaging/build_package.sh --builddir=\${build_dir}/test --install_deps=1
            bash -x ./valkey-ldap/packaging/build_package.sh --builddir=\${build_dir}/test --repo=${GIT_REPO} --branch=${GIT_BRANCH} --version=${VALKEY_LDAP_VERSION} --release=${VALKEY_LDAP_RELEASE} --use_local_packaging_script ${STAGE_PARAM}"
    """
}
...
pipeline {
    ...
    parameters {
        choice(
             ...
             name: 'CLOUD' )
        string(
            defaultValue: 'https://github.com/EvgeniyPatlan/valkey-ldap.git',
            description: 'URL for valkey-ldap repository',
            name: 'GIT_REPO')
        string(
        ...
        string(
            defaultValue: '1',
            description: 'valkey-ldap release value',
            name: 'VALKEY_LDAP_RELEASE')
        string(
            defaultValue: 'valkey-ldap',
            description: 'valkey-ldap repo name',
            name: 'VALKEY_LDAP_REPO')
        choice(
            choices: 'laboratory\ntesting\nexperimental',
    ...
    stages {
        stage('Create valkey-ldap source tarball') {
            agent {
            ...
            steps {
                ...
                sh '''
                   REPO_UPLOAD_PATH=$(grep "UPLOAD" test/valkey-ldap.properties | cut -d = -f 2 | sed "s:$:${BUILD_NUMBER}:")
                   AWS_STASH_PATH=$(echo ${REPO_UPLOAD_PATH} | sed  "s:UPLOAD/experimental/::")
                   echo ${REPO_UPLOAD_PATH} > uploadPath
                   echo ${AWS_STASH_PATH} > awsUploadPath
                   cat test/valkey-ldap.properties
                   cat uploadPath
                   cat awsUploadPath
                '''
                script {
        ...
        stage('Build valkey-ldap generic source packages') {
            parallel {
                stage('Build valkey-ldap generic source rpm') {
                    agent {
                ...
                stage('Build valkey-ldap generic source deb') {
                    agent {
                        label params.CLOUD == 'Hetzner' ? 'docker-x64-min' : 'docker-32gb'
                    }
                    steps {
        ...
        stage('Build valkey-ldap RPMs/DEBs') {
            parallel {
        ...
        stage('Push to public repository') {
            steps {
                // sync packages
                sync2ProdAutoBuild(params.CLOUD, VALKEY_LDAP_REPO, COMPONENT)
            }
        }
    }
    post {
