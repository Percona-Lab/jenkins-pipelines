...
def tidy_go_module(go_mod: Path, preferred_go_version: str) -> None:
    version_after_update = go_mod_version(go_mod)
    result = run(
        "go",
        "mod",
        "tidy",
        f"-go={preferred_go_version}",
        cwd=go_mod.parent,
        check=False,
    )
    if result.returncode == 0:
        print(f"Keeping Go directive at {preferred_go_version}")
        return

    print(
        f"Go {preferred_go_version} is incompatible with the selected dependency; "
        f"using {version_after_update}"
    )
    run("go", "mod", "edit", f"-go={version_after_update}", cwd=go_mod.parent)
    run("go", "mod", "tidy", cwd=go_mod.parent)
...
def main() -> None:
    ...
    preferred_go_version = go_mod_version(args.go_mod)
