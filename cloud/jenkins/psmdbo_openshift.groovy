...
pipeline {
    ...
    parameters {
        ...
        string(name: 'PLATFORM_VERSION', defaultValue: 'latest', description: 'OpenShift kubernetes version. If set to min or max, value will be automatically taken from release_versions file.')
        ...
        stage('Prepare Test Variables') {
            steps {
                script {
                    ...
                    // OpenShift cluster names feed a wildcard DNS subdomain with a strict length limit.
                    testVariables.cluster_name = "psmdbo-${testVariables.git_short_commit.take(6)}".toLowerCase()

                    libraries.dependencies.installOpenshiftCLI(testVariables.platform_version)
            ...
            script {
                ...
                clusters.each { clusterSuffix ->
                    try {
                        ...
                        libraries.openshift.shutdownCluster(clusterCfg)
                    } catch (err) {
