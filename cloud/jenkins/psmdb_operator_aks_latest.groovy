...
void prepareNode() {
    ...
    if ("$PLATFORM_VER" == "latest") {
        USED_PLATFORM_VER = sh(script: "az aks get-versions --location $location --output json | jq -r '.values | max_by(.patchVersions) | .patchVersions | keys[0]'", , returnStdout: true).trim()
    } else {
