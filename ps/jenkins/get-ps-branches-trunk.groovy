...
pipeline {
    ...
    stages {
       ...
       sh """
            EC=0
            aws s3 ls s3://percona-jenkins-artifactory/percona-server/branch_commit_id_trunk.properties || EC=\$?

            if [ \${EC} = 1 ]; then
              LATEST_RELEASE_BRANCH=\$(git -c 'versionsort.suffix=-' ls-remote --heads --sort='v:refname' ${GIT_REPO} trunk | tail -1)
              BRANCH_NAME=\$(echo \${LATEST_RELEASE_BRANCH} | cut -d "/" -f 3)
              COMMIT_ID=\$(echo \${LATEST_RELEASE_BRANCH} | cut -d " " -f 1)

              echo "BRANCH_NAME=\${BRANCH_NAME}" > branch_commit_id_trunk.properties
              echo "COMMIT_ID=\${COMMIT_ID}" >> branch_commit_id_trunk.properties

              aws s3 cp branch_commit_id_trunk.properties s3://percona-jenkins-artifactory/percona-server/
                          echo "START_NEW_BUILD=NO" > startBuild
              else
                          aws s3 cp s3://percona-jenkins-artifactory/percona-server/branch_commit_id_trunk.properties .
              source ./branch_commit_id_trunk.properties
              cat ./branch_commit_id_trunk.properties
              ...
              echo "BRANCH_NAME=\${LATEST_BRANCH_NAME}" > branch_commit_id_trunk.properties
              echo "COMMIT_ID=\${LATEST_COMMIT_ID}" >> branch_commit_id_trunk.properties
        ...
        stage('Build skipped') {
            ...
            steps {
                withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', accessKeyVariable: 'AWS_ACCESS_KEY_ID', credentialsId: 'AWS_STASH', secretKeyVariable: 'AWS_SECRET_ACCESS_KEY']]) {
                    sh """
                        echo ${START_NEW_BUILD}: NO build required
                    """
                }
            }
        }
