        ...
        case "DEB" :
             sh """
	            ...
	            PKGLIST=\\"\\\${PKGLIST} libmecab-dev libncurses5-dev libreadline-dev libpam-dev zlib1g-dev libcurl4-openssl-dev\\"
                    ...
                    if [ \\\$DEBIAN_VERSION = focal -o  \\\$DEBIAN_VERSION = bullseye -o \\\$DEBIAN_VERSION = jammy -o  \\\$DEBIAN_VERSION = bookworm -o \\\$DEBIAN_VERSION = trixie -o  \\\$DEBIAN_VERSION = noble -o \\\$DEBIAN_VERSION = resolute ]; then
		...
		stage('Ubuntu Noble (24.04) ARM') {
                ...
                stage('Ubuntu Resolute (26.04)') {
                    ...
                    steps {
                        cleanUpWS()
                        popArtifactFolder(params.CLOUD, "source_tarball/", AWS_STASH_PATH)
                        buildStage("ubuntu:resolute", "DEB")
                        sh '''
                            pwd
                            ls -la test/deb
                            cp -r test/deb .
