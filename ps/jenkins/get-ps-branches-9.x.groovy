    ...
    stages {
        stage('Get release branches') {
            steps {
                withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', accessKeyVariable: 'AWS_ACCESS_KEY_ID', credentialsId: 'AWS_STASH', secretKeyVariable: 'AWS_SECRET_ACCESS_KEY']]) {
                    sh """
                        EC=0
                        aws s3 ls s3://percona-jenkins-artifactory/percona-server/branch_commit_id_9x.properties || EC=\$?

            if [ \${EC} = 1 ]; then
              LATEST_RELEASE_BRANCH=\$(git -c 'versionsort.suffix=-' ls-remote --heads --sort='v:refname' ${GIT_REPO} release-9.\\* | tail -1)
              BRANCH_NAME=\$(echo \${LATEST_RELEASE_BRANCH} | cut -d "/" -f 3)
              COMMIT_ID=\$(echo \${LATEST_RELEASE_BRANCH} | cut -d " " -f 1)

              echo "BRANCH_NAME=\${BRANCH_NAME}" > branch_commit_id_9x.properties
              echo "COMMIT_ID=\${COMMIT_ID}" >> branch_commit_id_9x.properties

              aws s3 cp branch_commit_id_9x.properties s3://percona-jenkins-artifactory/percona-server/
                          echo "START_NEW_BUILD=NO" > startBuild
              else
                          aws s3 cp s3://percona-jenkins-artifactory/percona-server/branch_commit_id_9x.properties .
              source ./branch_commit_id_9x.properties
              cat ./branch_commit_id_9x.properties
              ...
              echo "BRANCH_NAME=\${LATEST_BRANCH_NAME}" > branch_commit_id_9x.properties
              echo "COMMIT_ID=\${LATEST_COMMIT_ID}" >> branch_commit_id_9x.properties
              echo "VERSION=\${VERSION}" >> branch_commit_id_9x.properties
                          aws s3 cp branch_commit_id_9x.properties s3://percona-jenkins-artifactory/percona-server/
                        fi
                    """
                }
                script {
                    ...
                    VERSION = sh(returnStdout: true, script: "source ./branch_commit_id_9x.properties; echo \${BRANCH_NAME} | cut -d - -f 2-3 ").trim()
                }
        ...
        stage('Build needed') {
            ...
            steps {
                ...
                build job: 'ps9.0-RELEASE', parameters: [string(name: 'BRANCH', value: BRANCH_NAME), string(name: 'COMPONENT', value: 'testing')]
