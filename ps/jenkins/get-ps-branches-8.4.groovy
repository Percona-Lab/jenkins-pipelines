        ...
        stage('Get release branches') {
            steps {
                withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', accessKeyVariable: 'AWS_ACCESS_KEY_ID', credentialsId: 'AWS_STASH', secretKeyVariable: 'AWS_SECRET_ACCESS_KEY']]) {
                    sh """
                        EC=0
                        aws s3 ls s3://percona-jenkins-artifactory/percona-server/branch_commit_id_84.properties || EC=\$?

            if [ \${EC} = 1 ]; then
              LATEST_RELEASE_BRANCH=\$(git -c 'versionsort.suffix=-' ls-remote --heads --sort='v:refname' ${GIT_REPO} release-8.4\\* | tail -1)
              ...
              echo "BRANCH_NAME=\${LATEST_BRANCH_NAME}" > branch_commit_id_84.properties
              echo "COMMIT_ID=\${LATEST_COMMIT_ID}" >> branch_commit_id_84.properties
              echo "VERSION=\${VERSION}" >> branch_commit_id_84.properties
                          aws s3 cp branch_commit_id_84.properties s3://percona-jenkins-artifactory/percona-server/
                        fi
                    """
                }
                script {
                    ...
                    BRANCH_NAME = sh(returnStdout: true, script: "source ./branch_commit_id_84.properties; echo \${BRANCH_NAME}").trim()
                    COMMIT_ID = sh(returnStdout: true, script: "source ./branch_commit_id_84.properties; echo \${COMMIT_ID}").trim()
                    VERSION = sh(returnStdout: true, script: "source ./branch_commit_id_84.properties; echo \${BRANCH_NAME} | cut -d - -f 2-3 ").trim()
                    TOKUBACKUP_BRANCH = sh(returnStdout: true, script: "source ./branch_commit_id_84.properties; echo Percona-Server-\${VERSION}").trim()
                    PERCONAFT_BRANCH = sh(returnStdout: true, script: "source ./branch_commit_id_84.properties; echo Percona-Server-\${VERSION}").trim()
                }
